...
// Verifica si una cuenta está bloqueada leyendo el archivo bloqueadas.dat
bool estaBloqueada(const char* numero) {
    FILE* archivo = fopen("bloqueadas.dat", "rb");
    if (!archivo) return false; // Si el archivo no existe, no hay cuentas bloqueadas aún

    char cuenta[7];
    while (fread(cuenta, sizeof(cuenta), 1, archivo) == 1) {
        if (strcmp(cuenta, numero) == 0) {
            fclose(archivo);
            return true; // Coincidencia encontrada, la cuenta está bloqueada
        }
    }
...
// Bloquea una cuenta agregando su número al archivo bloqueadas.dat
void bloquearCuenta(const char* numero) {
    FILE* archivo = fopen("bloqueadas.dat", "ab");
    if (archivo) {
        fwrite(numero, sizeof(char[7]), 1, archivo); // Guardamos el número de cuenta bloqueada
        fclose(archivo);
    }
}
    ...
    do {
        ...
        switch (opcion) {
            ...
            case 2: {
    			...
    			if (estaBloqueada(numero)) {
        			cout << "\n";
        			color(4);
        			centrarTexto("Esta cuenta se encuentra BLOQUEADA por múltiples(3) intentos fallidos.\n");
        			color(15);
        			Sleep(3000);
        			system("cls");
        			titulo();
    			...
    			// Proceso de ingreso de clave con 3 intentos
    			int intentos = 0;
    			bool acceso = false;
    			...
    			if (!acceso) {
        			color(4);
			        centrarTexto("Ha superado el número máximo de intentos.\n");
			        cout<<"\n";
			        centrarTexto("La cuenta será BLOQUEADA por seguridad.\n");
			        color(15);
			        bloquearCuenta(numero);
			        Sleep(5000);
			        system("cls");
			        titulo();
			        break;
